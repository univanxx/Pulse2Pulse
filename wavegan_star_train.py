...
parser.add_argument('--fold_idx', default=2, help="Fold number")
...
def train(netG, netD, optimizerG, optimizerD, dataloader):
    ...
    pbar = tqdm(range(opt.start_epoch+1, opt.num_epochs+1))
    for epoch in pbar:
        ...
        for i, sample in tqdm(enumerate(dataloader, 0)):
            ...
            # Noise
            noise = torch.Tensor(b_size, 100).uniform_(-1, 1)
        ...
        if epoch % opt.checkpoint_interval == 0:
            ...
            for label_i in [0,1]:
                ...
                noise = torch.Tensor(b_size, 100).uniform_(-1, 1).to(device)
                with torch.no_grad():
...
def save_model(netG, netD, optimizerG, optimizerD,  epoch):
    ...
    check_point_name = "gan" + "_epoch:{}.pt".format(epoch) # get code file name and make a name
